...
```{r}
# Windowing perché segnale complessivo non periodico -> FFT non funziona
N <- length(y$t)
y <- y %>% mutate(
...
## Provo con padding simmetri a zero [ChatGPT]
...
Applico un filtro sul segnale padded:
...
u_comp %>% plot_ly(x = ~t) %>% 
  ...
  add_lines(y = u$u[u$t <= Ta], name = "Ideale") %>%
  add_lines(y = ~u_pad, name = "Padded") %>%
  add_lines(y = ~u_flt, name = "Filtered")
```


ora accetto il risultato