...
L'applicazione del un filtro online introduce un ritardo rispetto al segnale originale, infatti la fase della FdT non è sempre pari a zero. Per sopperire a tale ritardo si può [filtrare avanti e indietro](https://mmt-unitn.github.io/teoria.html#funzioni-di-trasferimento) nel tempo: in R questo filtro a fase zero è realizzato mediante la funzione `signal::filtfilt`. Tuttavia, sarà necessario affrontare un problema di **distorsione nelle code** del segnale, causato da un **padding automatico** della funzione.
