...
pp <- plot_ly() %>%
  add_lines(x = sig_art$t, y = sig_art$p_art_n, line = list(color = "blue"), name = "Con rumore") %>%
  add_lines(x = sig_art$t, y = sig_art$p_art_k, line = list(color = "red"), name = "Nominale") %>%
...
# filtro digitale
flt <- butter(ord, W=fc/fn, plane="z")

p1 <- sig_art %>% select(-p_art, -p_art_k) %>% rename(p = p_art_n) %>% 
  mutate(p_flt = signal::filter(flt, p)) %>%
  pivot_longer(
    cols = c(p, p_flt)
  ) %>%
...
p2 <- sig_art %>% select(-p_art, -p_art_k) %>% rename(p = p_art_n) %>% 
  mutate(p_flt = signal::filter(flt, p)) %>%
  pivot_longer(
    cols = c(p, p_flt)
  ) %>%
...
sig_art %>% select(-p_art, -p_art_k) %>% rename(p = p_art_n) %>% mutate(
  p_flt = signal::filter(flt, p)
) %>% plot_ly(x = ~t) %>%
  add_lines(y = ~p, line = list(color = "blue"), name = "Con rumore") %>%
  add_lines(y = ~p_flt, line = list(color = "magenta"), name = "Filtrato") %>%
...
p1 <- ggbodeplot_digital(flt, fs, xaxis = "omega")
p2 <- ggbodeplot_digital(flt, fs, fmin = 1e-1, fmax = 1e2) + geom_vline(xintercept = fc, linetype = 2, color = "red")
...
L'applicazione di un filtro offline introduce un ritardo rispetto al segnale originale. Per sopperire a tale ritardo si può **filtrare avanti e indietro** nel tempo: in R questo filtro a fase zero è realizzato mediante la funzione `signal::filtfilt`. Tuttavia, sarà necessario affrontare un problema di **distorsione nelle code** del segnale, causato da un **padding automatico** della funzione.

```{r}
sig_art %>% select(-p_art, -p_art_k) %>% rename(p = p_art_n) %>% mutate(
  p_flt = signal::filtfilt(flt, p)
) %>% plot_ly(x = ~t) %>%
  add_lines(y = ~p, line = list(color = "blue"), name = "Con rumore") %>%
  add_lines(y = ~p_flt, line = list(color = "magenta"), name = "Filtrato") %>%
  layout(xaxis = list(title = "Tempo (s)"), yaxis = list(title = "Pressione (mmHg)"), title = "Filtraggio con taglio a 5 Hz")
...
p_filtered <- sig_art %>% select(-p_art, -p_art_k) %>% rename(p = p_art_n) %>% 
  mutate(p_flt = signal::filtfilt(flt, p)) %>%
