...
ym.flt <- ym.flt %>% select(-fft, -intensity, -phase, -f) %>% mutate(
  y_mix = (2*yn_m_n.flt*cos(2*pi*fp*t) - 1)/m # demodulazione sincrona in un passo
)

# Filtro passa-basso:
# usato per attenuare la portante (alta frequenza)
# e per attenuare il rumore bianco -> che dallo spettro vedo essere minuscolo
f_cut2 <- 2*max(pars_m$f) # per garantire l'intero segnale misurato
...
ym.flt %>% plot_ly(x = ~t) %>%
  ...
  layout(xaxis = list(title="Tempo (s)"), yaxis=list(title="Segnale"))
...
ym.flt %>% pivot_longer(cols = c("yn", "y_flt2")) %>%
  ...
  geom_line() +
  scale_colour_discrete(labels = c(yn = "Misurato", y_flt2 = "Demodulato")) +
  labs(x = "Tempo (s)", y = "Segnale", colour = "Segnale")
```

notare come il filtraggio pb abbia ridotto nche il rumore bianco
anche la riduzione di ampiezza per via della rielaborazione
